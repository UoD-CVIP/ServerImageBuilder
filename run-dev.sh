...
do
  ...
  then
    BASE_IMAGES=$( populate_image_list ./tensorflow_image_tags.txt.dev "tensorflow/tensorflow" )
    BUILD_DIR=./TensorflowBuild
    MATLAB_BUILD_FLAG=false

  elif [[ ${FRAMEWORK} == "pytorch" ]]
  then
    BASE_IMAGES=$( populate_image_list ./pytorch_image_tags.txt.dev "pytorch/pytorch" )
    BUILD_DIR=./PyTorchBuild
    MATLAB_BUILD_FLAG=false

  elif [[ ${FRAMEWORK} == "matlab" ]]
  then
    BASE_IMAGES=$( populate_image_list ./matlab_image_tags.txt.dev "nvcr.io/partners/matlab" )
